...
class Task():
    ...
    def run(self):
        ...
        if args.local:
            ...
            cmd = 'docker run -e GITHUB_OAUTH="%s" --memory=5g -v %s:/home/jdbl/results --rm jdbl debloat --output /home/jdbl/results -d https://github.com/%s.git --lib-commit %s -c https://github.com/%s.git -v %s' % (token, OUTPUT, self.library['repo_name'], self.lib_commit, self.client['repo_name'], self.version)
