...
raw_results <- read.delim("Data/raw_results_final_4.csv", sep = ",", header = T, stringsAsFactors = TRUE)
...
results %>% 
  select(ID, Debloat, Lib, Lib.original.test.failing, Lib.original.test.error, Debloat.Time) %>% 
  filter(Debloat == "True" & Lib.original.test.failing == 0 & Lib.original.test.error == 0) %>% 
  select(ID) %>% 
  distinct()
...
results %>% filter(Debloat == "True") %>% group_by(ID) %>% summarize(n = n())
