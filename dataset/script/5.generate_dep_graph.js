...
async.eachLimit(
    ...
    pomParser.parse(
      ...
      function (err, pomResponse) {
        ...
        if (mapIdToInfo[parentId]) {
          const previousRepo = JSON.parse(
            fs.readFileSync(
              config.output +
                "repo_details/" +
                mapIdToInfo[parentId].repo_name +
                ".json"
            )
          );
          const newRepo = JSON.parse(
            fs.readFileSync(config.output + "repo_details/" + repo + ".json")
          );
  ...
  async () => {
    await fs.writeFileSync(
      config.output + "repoInfo.json",
      JSON.stringify(mapIdToInfo)
    );
    const output = {};
    ...
    //     const dep = output[id]
    //     for (let version in dep.clients.versions) {
    //         dep.clients.versions[version].sort((a, b) => {
    ...
    fs.writeFileSync(
      config.output + "maven_graph.json",
